...
public interface GroupApiService {
    ...
    Observable<Resource<ArrayList<WiseLiUser>>> getFriendsListCircle1(@Nullable @Query("token") String token,
                                                                      @Nullable @Query("circle_id") Integer circleId);

    @GET(API_GET_CIRCLE_DETAILS)
    Observable<Resource<ArrayList<WiseLiUser>>> getCircleDetails2(@Nullable @Query("token") String token,
                                                                  @Nullable @Query("circle_id") Integer circleId);

    @FormUrlEncoded
    @POST(API_REMOVE_CIRCLE_USER)
    Observable<Resource<WiseLiUser>> removeCircleUser3(@Nullable @Field("token") String token,
                                                       @Nullable @Field("circle_id") Integer circleId,
                                                       @Nullable @Field("user_id") Integer userID);

    @FormUrlEncoded
    @POST(API_ACTIVE_LIST)
    Observable<Resource<WiseLiUser>> getActiveList(@Nullable @Field("token") String token,
                                                   @Nullable @Field("circle_id") Integer propertyID);
